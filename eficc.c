...
License at the time of copying can be found in lic/chibicc.license.txt
Modified by Jonathan M. Wilbur <jonathan@wilbur.space>. */
...
unsigned long int strtoul(char *str, char **endptr, int base) {
    ...
    while (*str != 0) {
        if (
            (base <= 10 && !isdigit(*str)) || (base > 10 && !isxdigit(*str))) {
            goto end;
        }
...
void assert_fail(const char *file, const int line) {
    // TODO: Do something here.
    return;
}

void assert_pass(void) {
    return;
}

#define assert(expr) ((expr) ? assert_pass() : assert_fail(__FILE__, __LINE__))
...
static bool function_name_used = false;
...
noreturn static int die(int exit_code, const char* fn_name) {
    write(STDERR_FILENO, fn_name, strlen(fn_name));
    if (errno != 0)
        perror(" chibicc @ die");
    cleanup();
    exit(exit_code);
...
uint32_t read_ascii_byte(char **new_pos, char *p);
...
static void emit_data(Obj *prog) {
    for (Obj *var = prog; var; var = var->next) {
        ...
        if (var->is_static)
            println("  .local %s", var->asm_name != NULL ? var->asm_name : var->name);
        else
            println("  .globl %s", var->asm_name != NULL ? var->asm_name : var->name);
        ...
        if (var->init_data) {
            ...
            bool is_null_term = var->ty->size > 0 && var->init_data[var->ty->size - 1] == 0;
            bool is_null_term_ascii = false;
            if (is_null_term) {
                is_null_term_ascii = true;
                for (int i = 0; i < var->ty->size - 1; i++) {
                    char c = var->init_data[i];
                    if (!isprint(c) || c == '"') {
                        is_null_term_ascii = false;
                        break;
                    }
                } 
            }
            if (is_null_term_ascii) {
                println("  .asciz \"%s\"", var->init_data); // A short-cut to make our assembly cleaner.
            } else {
                int pos = 0;
...
static void emit_text(Obj *prog) {
    for (Obj *fn = prog; fn; fn = fn->next) {
        ...
        char *name = (fn->asm_name != NULL) ? fn->asm_name : fn->name;
        if (fn->is_static)
        ...
        if (fn->va_area) { // d12546e9-1efd-4aae-937a-42144c560004
            int gp = 0, fp = 0;
            for (Obj *var = fn->params; var; var = var->next) {
                if (is_flonum(var->ty))
                    fp++;
...
static Token *parse_function(Token *tok, Type *basety, VarAttr *attr);
...
static Type *func_params(Token **rest, Token *tok, Type *ty, int nameless_params) {
    ...
    // TODO: Create a PR to add this comment.
    /* This behavior is documented here: https://github.com/rui314/chibicc/issues/46
    In C, foo() is different from foo(void) in that the former has an
    unspecified parameters list; the latter explicitly takes no parameters.
    This code is "playing it safe" and pushing all arguments in registers to the
    stack in the setup of the stack frame.
    Search this codebase for d12546e9-1efd-4aae-937a-42144c560004 to see other
    relevant code. */
    if (cur == &head) // If we parsed no parameters...
        is_variadic = true; // ...treat the function as variadic.
...
static Node *compound_stmt(Token **rest, Token *tok) {
    ...
    while (!equal(tok, "}")) {
        if (is_typename(tok) && !equal(tok->next, ":")) {
            ...
            if (is_function(tok)) {
                tok = parse_function(tok, basety, &attr);
                continue;
            }
...
Token *attribute_list(Token *tok, Type *ty) {
    while (consume(&tok, tok, "__attribute__")) {
        ...
        while (!consume(&tok, tok, ")")) {
            ...
            if (
                (consume(&tok, tok, "optimize") || consume(&tok, tok, "__optimize__")) && (consume(&tok, tok, "("))) {
                while (!consume(&tok, tok, ")")) {
                    tok = tok->next;
                    // consume(&tok, tok, ",");
...
static Token *parse_function(Token *tok, Type *basety, VarAttr *attr) {
    ...
    if (
        (consume(&tok, tok, "__asm__") || consume(&tok, tok, "asm")) && consume(&tok, tok, "(") && tok->kind == TK_STR) {
    ...
    // TODO: Would this mean that it's just a declaration? Research and document.
    if (consume(&tok, tok, ";"))
        return tok;
    ...
    if (ty->is_variadic) // d12546e9-1efd-4aae-937a-42144c560004
    ...
    // If __func__ or __FUNCTION__ is used anywhere in the code, the function
    // names are all added to the binary, resulting in a huge size increase of
    // the data section.
    if (function_name_used) { // TODO: Create a PR for this.
        Obj *func = new_string_literal(fn->name, array_of(ty_char, strlen(fn->name) + 1));
        // TODO: Create a PR to de-dupe these.
        push_scope("__func__")->var = func;
        // [GNU] __FUNCTION__ is yet another name of __func__.
        push_scope("__FUNCTION__")->var = func;
    }
...
static int read_ident(char *start) {
    char *p = start;
    uint32_t c = read_ascii_byte(&p, p);
    if (!is_ident1(c))
        return 0;

    for (;;) {
        char *q;
        c = read_ascii_byte(&q, p);
        if (!is_ident2(c))
            return p - start;
        p = q;
    }
...
Token *tokenize(File *file) {
    ...
    while (*p) {
        ...
        if (ident_len) {
            /* The use of these macros greatly increases the size of the data
            section, so we check if we actually need it first. */
            if (
                !function_name_used
                && ident_len >= 8
                && !strncmp(p, "__func__", ident_len)
                || !strncmp(p, "__FUNCTION__", ident_len)
            ) {
                function_name_used = true;
            }
...
static void canonicalize_newline(char *p) {
    int i = 0; // Input character index into the string p.
    int j = 0; // Output character index into the string p.
...
static void remove_backslash_newline(char *p) {
    /* As a backslash-escaped newline is unescaped, the string effectively
    becomes one character shorter; the string BACKSLASH + N is two characters, but a
    newline is just a single character. Since the string will get shorter, we
    can just write the unescaped output characters back into itself. i is the
    index into the original string, and j is the index where we are writing. */
    int i = 0; // Input character index into the string p.
    int j = 0; // Output character index into the string p.
...
// This does not accept C11 multibyte characters. ASCII only for simplicity.
bool is_ident1(uint32_t c) {
    static uint32_t range[] = { '_', '_', 'a', 'z', 'A', 'Z', -1 };
    return in_range(range, c);
}

// Returns true if a given character is acceptable as a non-first
// character of an identifier. Does not accept C11 multibyte characters.
// ASCII only for simplicity.
bool is_ident2(uint32_t c) {
    static uint32_t range[] = { '0', '9', '$', '$', -1 };
...
static void parse_args(int argc, char **argv) {
    ...
    output_file = fopen("asdf.out", "w+"); // TODO: Change later.
